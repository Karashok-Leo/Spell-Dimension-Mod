...
public class AdditionalTooltip
{
    ...
    {
        if (!stack.isOf(AllItems.FLICKER_BREASTPLATE))
        {
            return;
        }
        var player = MinecraftClient.getInstance().player;
    ...
    {
        if (!stack.isOf(AllItems.SPIRIT_TOME))
        {
            return;
        }
    ...
    {
        if (!(stack.getItem() instanceof ModonomiconItem))
        {
            return;
        }
    ...
    {
        if (!stack.isOf(AllItems.BOTTLE_NIGHTMARE))
        {
            return;
        }
        ClientPlayerEntity player = L2HostilityClient.getClientPlayer();
