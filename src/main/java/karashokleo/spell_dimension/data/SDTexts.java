...
public enum SDTexts
{
    ...
    TEXT$SPIRIT_TOME$RULE$2("Rule 2: Gain \"Negative Spirit\" upon defeating hostile mobs", "规则二：击杀恶意生物时获得「负通灵」"),
    ...
    TEXT$SPIRIT_TOME$RULE$6("Rule 6: When \"Positive Spirit\" exceeds 1,000,000 and \"Negative Spirit\" is less than 10,000, acquire the ability \"Disarm\": Enemies with health below 10% become soul minions", "规则六：「正通灵」大于百万且「负通灵」小于一万时，获得能力「止戈」：敌人生命值低于一成时转为灵仆"),
    TEXT$SPIRIT_TOME$RULE$7("Rule 7: When \"Negative Spirit\" exceeds 1,000,000 and \"Positive Spirit\" is less than 10,000, acquire the ability \"Judgment\": Enemies with health below 10% are executed", "规则七：「负通灵」大于百万且「正通灵」小于一万时，获得能力「裁决」：敌人生命值低于一成时即刻斩杀"),
    TEXT$SPIRIT_TOME$RULE$REVEAL("A new rule has been revealed!", "新的规则已揭示！"),
    // rule effect messages
    TEXT$SPIRIT_TOME$RULE$0$EFFECT("Rule 0 activated. Erase immediately!", "规则〇执行，即刻抹杀！"),
    ...
    TOOLTIP$SOUL_CONTAINER$USAGE_1("Right-click and hold on non-boss mobs with health below %s or bosses with health below %s to attempt capturing their souls, converting them into soul minions and storing them in containers. The lower the target's health, the higher the success rate.", "对生命值低于%s的非Boss生物或生命值低于%s的Boss生物长按右键可尝试捕捉其灵魂，转化为灵仆并存储在容器中，目标生命值越低成功率越高。"),
    ...
    TOOLTIP$CONTAINER$ILLUSION_UPGRADE("Attempt to pick up and void convertible items (without amplified by Enchanted Essences) and their enchantments, converting them into materials of the same rarity and the Book of Omniscience.", "尝试拾取并销毁可转化的（未被束魔精华强化过的）物品及其附魔，转化为同等级材料和全知之书"),
    ...
    TOOLTIP$REJUVENATING_BLOSSOM$1("- +%s Healing amount for every 1 healing spell power", "- 每点治愈法术强度 +%s 治疗量"),
    TOOLTIP$REJUVENATING_BLOSSOM$2("Current: +%s Healing amount", "当前效果：+%s 治疗量"),
    ...
    TOOLTIP$ATOMIC_TO_ENCHANTED("After dealing a total of %s magic damage, can be upgraded to Enchanted Breastplate", "总共造成%s点魔法伤害后可升级为束魔护心镜"),
    TOOLTIP$ATOMIC_TO_FLEX("After suffering a total of %s damage, can be upgraded to Flex Breastplate", "总共承受%s点伤害后可升级为曲御护心镜"),
    TOOLTIP$ATOMIC_TO_FLICKER("After rolling a total of %s times, can be upgraded to Flicker Breastplate", "总共翻滚%s次后可升级为闪曳护心镜"),
    TOOLTIP$ATOMIC_TO_OBLIVION("After restoring with a total of %s health, can be upgraded to Oblivion Breastplate", "总共恢复%s点生命值后可升级为湮灭护心镜"),
    ...
    TOOLTIP$CURSE_PRIDE$1("All school spell power are increased by %s%% per experience level", "根据玩家经验等级，所有学派法术强度每级提升%s%%"),
    TOOLTIP$CURSE_PRIDE$2("Current spell power increment: +%s", "当前增幅：+%s"),
