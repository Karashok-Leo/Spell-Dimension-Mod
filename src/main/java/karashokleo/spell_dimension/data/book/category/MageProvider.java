...
public class MageProvider extends CategoryProvider
{
    ...
    protected String[] generateEntryMap()
    {
        return new String[]{
                ...
                "_____________________",
                "_____________________",
                "______u___c___e______",
                "_____________________",
                "_____________________",
                "_____________________",
                "______k___n___r______",
                "_____________________",
        };
    }
    ...
    protected void generateEntries()
    {
        ...
        BookEntryModel school = this.entrySchool().withParent(cast);
        BookEntryModel rune = this.entryRune().withParent(cast);
        BookEntryModel essence = this.entryEssence().withParent(cast);
        BookEntryModel bind = this.entryBind().withParent(cast);
        BookEntryModel book = this.entryBook().withParent(bind);
        BookEntryModel scroll = this.entryScroll().withParent(bind);
        this.add(cast);
        this.add(school);
        this.add(rune);
        this.add(essence);
    ...
    private BookEntryModel entryEssence()
    {
        ...
        context.page("enchanted");
        this.lang().add(context.pageTitle(), "Enchanted Essence");
        this.lang().add(context.pageText(), "Enchanted Essence can imbue an item with magic power to increase some of its attributes. Be aware that Enchanted Essence can only work on an item if its Threshold is not less than the item's Enchanted Level. Primary, Intermediate, and Advanced Basic Essences can be crafted into Intermediate Essences with a Threshold of 10, 20, and 30 respectively. The maximum Threshold of Enchanted Essence that can be obtained from loot is 60.");
        this.lang("zh_cn").add(context.pageTitle(), "束魔精华");
        this.lang("zh_cn").add(context.pageText(), "束魔精华可以为物品注入魔力，使其增加部分属性。要注意只有当束魔精华的阈值不小于物品的束魔等级时才能作用于该物品。初级、中级、高级的基础精华分别可以合成阈值为10、20、30的束魔精华。战利品中最高能获取到阈值为60的束魔精华。");

        BookSpotlightPageModel enchanted = BookSpotlightPageModel
                .builder()
                .withTitle(context.pageTitle())
                .withText(context.pageText())
        ...
        context.page("enlightening");
        this.lang().add(context.pageTitle(), "Enlightening Essence");
        this.lang().add(context.pageText(), "Enlightening Essence is similar to Enchanted Essence, except that while Enchanted Essence works on items, Enlightening Essence works on the mage himself and is not limited by a threshold. Enlightening Essence cannot be crafted and can only be obtained from loot.");
        this.lang("zh_cn").add(context.pageTitle(), "源启精华");
        this.lang("zh_cn").add(context.pageText(), "源启精华与束魔精华类似，不同的是束魔精华作用于物品，而源启精华作用于魔法师自身，且不受限于阈值。源启精华不能合成，只能从战利品中获取。");

        BookSpotlightPageModel enlightening = BookSpotlightPageModel
                .builder()
                .withTitle(context.pageTitle())
                .withText(context.pageText())
        ...
        context.page("disenchanted");
        this.lang().add(context.pageTitle(), "Disenchanted Essence");
        this.lang().add(context.pageText(), "Disenchanted Essence removes all the magic infused into an item by Enchanted Essences. The only way to obtain an Disenchanted Essence is to purchase it from a Goblin Trader.");
        this.lang("zh_cn").add(context.pageTitle(), "祛魔精华");
        this.lang("zh_cn").add(context.pageText(), "祛魔精华能够消除束魔精华注入在一件物品上的所有魔力。获取祛魔精华的唯一途径是从哥布林商人手中购买。");

        BookSpotlightPageModel disenchanted = BookSpotlightPageModel
                .builder()
                .withTitle(context.pageTitle())
                .withText(context.pageText())
        ...
        this.lang().add(context.pageText(), "Mending Essence completely repairs an item and removes its accumulated repair penalties. Mending Essence cannot be crafted and can only be obtained from loot.");
        this.lang("zh_cn").add(context.pageTitle(), "修复精华");
        this.lang("zh_cn").add(context.pageText(), "修复精华可以完全修复一件物品, 并且清除其累积的修复惩罚。修复精华不能合成，只能从战利品中获取。");
        ...
        return this.entry('e')
                ...
                .withPages(base, enchanted, enlightening, disenchanted, mending);
    }
    ...
    private BookEntryModel entryBook()
    {
        ...
        BookSpotlightPageModel book = BookSpotlightPageModel
                .builder()
                .withTitle(context.pageTitle())
                .withText(context.pageText())
    ...
    private BookEntryModel entryScroll()
    {
        ...
        this.lang().add(context.pageText(), "There are a limited number of spells that you can learn through the Spell Binding Table. To learn to use more spells, you need to have Spell Scrolls. Spell Scrolls can be used directly to cast spells, or they can be freely bound or unbound to Spell Books. Spell Scrolls are usually obtained by killing specific monsters or crafted from specific materials.");
        this.lang("zh_cn").add(context.pageTitle(), "法术卷轴");
        this.lang("zh_cn").add(context.pageText(), "通过法术绑定台能够学习到的法术有限。要想学习使用更多魔法，你需要拥有法术卷轴。法术卷轴可以直接用于施法，也可以自由地与法术书绑定或解绑。法术卷轴通常需要通过击杀特定的怪物获取，或是用特定的材料合成。");

        BookSpotlightPageModel scroll = BookSpotlightPageModel
                .builder()
                .withTitle(context.pageTitle())
                .withText(context.pageText())
