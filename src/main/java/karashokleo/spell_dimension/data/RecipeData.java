...
import karashokleo.spell_dimension.content.item.SpellBooksEntry;
...
import karashokleo.spell_dimension.content.recipe.EnchantedEssenceRecipeJsonProvider;
import karashokleo.spell_dimension.content.recipe.MedalUpgradeRecipeJsonProvider;
...
public class RecipeData extends FabricRecipeProvider
{
    ...
    private static void addSpellBookRecipe(Consumer<RecipeJsonProvider> exporter)
    {
        for (SpellBooksEntry entry : AllItems.SPELL_BOOKS.values())
            for (Map.Entry<MageMajor, List<MageSpellBookItem>> listEntry : entry.majors.entrySet())
            {
                ...
                Identifier id = SpellDimension.modLoc(SchoolUtil.getName(major.school) + "/" + major.majorName() + "/spell_book/");
    ...
    private static void addMageMedalRecipe(Consumer<RecipeJsonProvider> exporter)
    {
        ...
        {
            Identifier id = SpellDimension.modLoc(SchoolUtil.getName(major.school) + "/" + major.majorName() + "/medal/");
        ...
        for (SpellSchool school : SchoolUtil.SCHOOLS)
        {
            for (int i = 0; i < 2; i++)
            {
                ...
                ShapelessRecipeJsonBuilder
                        ...
                        .offerTo(exporter, SpellDimension.modLoc(SchoolUtil.getName(school) + "/compose_" + i));
                ...
                ShapelessRecipeJsonBuilder
                        ...
                        .criterion(FabricRecipeProvider.hasItem(sub), FabricRecipeProvider.conditionsFromItem(sub))
                        .criterion(FabricRecipeProvider.hasItem(sur), FabricRecipeProvider.conditionsFromItem(sur))
                        .offerTo(exporter, SpellDimension.modLoc(SchoolUtil.getName(school) + "/decompose_" + i));
            }
        }
    }
        ...
        for (SpellSchool school : SchoolUtil.SCHOOLS)
        {
            for (int i = 0; i < 3; i++)
                for (EquipmentSlot slot : EquipmentSlot.values())
                    exporter.accept(new EnchantedEssenceRecipeJsonProvider(SpellDimension.modLoc(SchoolUtil.getName(school) + "/" + slot.getName() + "/" + i), i, (i + 1) * 10, slot, school));
        }
    }
}